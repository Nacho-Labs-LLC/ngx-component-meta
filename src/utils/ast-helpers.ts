...
export function getStringProperty(
  ...
  for (const prop of obj.properties) {
    ...
    if (!ts.isIdentifier(prop.name) || prop.name.text !== propertyName)
      continue;
...
export function getDecoratorStringArg(
  decorator: DecoratorInfo,
): string | undefined {
  if (!decorator.args?.length) return undefined;
  const first = decorator.args[0];
...
export function isProtectedMember(member: ts.ClassElement): boolean {
  if (ts.isPropertyDeclaration(member) || ts.isMethodDeclaration(member)) {
    const modifiers = ts.getModifiers(member);
    return (
      modifiers?.some((m) => m.kind === ts.SyntaxKind.ProtectedKeyword) ?? false
    );
  }
  return false;
}
...
function getNewExpressionName(
  ...
  if (!prop.initializer || !ts.isNewExpression(prop.initializer))
    return undefined;
